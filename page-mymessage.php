<form action="http://localhost/jobs/mymessage/" method="post">
<?php
...
<?php
	...
	foreach($results as $perresult){
        ...
        $querypermess = "select * from message where author=$user_ID and isread=0 and post_id=$post_id";
		...
		foreach($resultspermess as $permess){
            ...
            echo '<div class="msg_root" id="root2'.$ia_2.'_">';
    			echo '<div class="msg_title" id="root2'.$ia_2.'">'.$permess->name.'</div>';
                echo '<div class="msg_body">';
    			echo $permess->email.'<br/>';
                echo $permess->message.'<br/>';
                echo '</div>';
            echo '</div>';
...
?>
 ...
 <input id="mess_id" name="mess_id" value="" type="hidden" />
  ...
  $(function() {
    ...
    //hi?n mess click
    var id_mss=$("#mess_id").val();
    $("#msg_root"+id_mss+" .msg_body").show();
