		...
		<?php if (have_posts()) : ?>
			...
			<?php while (have_posts()) : the_post(); ?>
				...
				<div class="section_content">
					...
					<p class="meta"><em><?php the_taxonomies(); ?> <?php if (!jr_check_expired($post) && jr_remaining_days($post)!='-') : ?><?php _e('Job expires in', APP_TD) ?> <strong><?php echo jr_remaining_days($post); ?></strong>.<?php endif; ?> <?php _e('Date Posted',APP_TD); ?> <?php echo date_i18n(__('j M',APP_TD), strtotime($my_query->post->post_date)); ?></strong> <span class="year"><?php echo date_i18n(__('Y',APP_TD), strtotime($my_query->post->post_date)); ?></em></p>
				...
				<?php do_action("adding_notes_box",$post->ID); ?>
