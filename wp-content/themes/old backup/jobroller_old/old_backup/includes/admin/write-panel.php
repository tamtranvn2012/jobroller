...
class JR_Job_Meta extends APP_Meta_Box{
	...
	public function form(){
		...
		if ( 'yes' == get_option('jr_submit_how_to_apply_display') ) {
			$meta_boxes[] = array(
				'title' => __('How to Apply', APP_TD),
				'type' => 'textarea',
				'name' => '_how_to_apply',
				'desc' => html('p', html( 'em', __('Details on how to apply to the job.', APP_TD) ) ),
				'extra' => array(
					'rows' => 4,
					'cols' => 55,
				),
...
class JR_Job_Pricing_Meta extends APP_Meta_Box{
	...
	public function before_display( $form_data, $post ){
		...
		$form_data['_blank_job_listing_start_date'] = $post->post_date;
		...
		if( ! empty( $form_data[JR_ITEM_FEATURED_LISTINGS.'_start_date'] ) ) {
			$form_data['_blank_'.JR_ITEM_FEATURED_LISTINGS.'_start_date'] = mysql2date( $date_format, $form_data[JR_ITEM_FEATURED_LISTINGS.'_start_date']);
		...
		if( ! empty( $form_data[JR_ITEM_FEATURED_CAT.'_start_date'] ) ) {
			$form_data['_blank_'.JR_ITEM_FEATURED_CAT.'_start_date'] = mysql2date( $date_format, $form_data[JR_ITEM_FEATURED_CAT.'_start_date']);
	...
	?>
		<script type="text/javascript">
			jQuery(function($){
				createExpireHandler( undefined, $("#<?php echo JR_JOB_DURATION_META; ?>"), $("#_blank_<?php echo APP_POST_TYPE; ?>_start_date"), $(''), $("#_blank_expire_<?php echo APP_POST_TYPE; ?>"), $ );
				$("#_blank_<?php echo APP_POST_TYPE; ?>_start_date").parent().parent().parent().hide();

				createExpireHandler( $("#<?php echo JR_ITEM_FEATURED_LISTINGS; ?>"), $("#<?php echo JR_ITEM_FEATURED_LISTINGS; ?>_duration"), $("#<?php echo JR_ITEM_FEATURED_LISTINGS; ?>_start_date"), $("#_blank_<?php echo JR_ITEM_FEATURED_LISTINGS; ?>_start_date"), $("#_blank_expire_<?php echo JR_ITEM_FEATURED_LISTINGS; ?>"), $ );
				$( "#_blank_<?php echo JR_ITEM_FEATURED_LISTINGS; ?>_start_date" ).datepicker({
				...
				createExpireHandler( $("#<?php echo JR_ITEM_FEATURED_CAT; ?>"), $("#<?php echo JR_ITEM_FEATURED_CAT; ?>_duration"), $("#<?php echo JR_ITEM_FEATURED_CAT; ?>_start_date"), $("#_blank_<?php echo JR_ITEM_FEATURED_CAT; ?>_start_date"), $("#_blank_expire_<?php echo JR_ITEM_FEATURED_CAT; ?>"), $ );
				$( "#_blank_<?php echo JR_ITEM_FEATURED_CAT; ?>_start_date" ).datepicker({
					dateFormat: "<?php echo $ui_display_format; ?>",
	...
	public function form(){
		...
		$output = array(
			...
			array(
				...
				'type' => 'text',
				'name' => '_blank_job_listing_start_date',
			),
			array(
	...
	function before_save( $data, $post_id ){
		...
		unset( $data['_blank_job_listing_start_date'] );
		unset( $data['_blank'] );
