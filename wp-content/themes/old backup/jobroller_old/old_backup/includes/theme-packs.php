...
function jr_is_expired_user_pack( $user_id, $plan_umeta_id ) {
	...
	if ( $end_date > 0 && strtotime($end_date) > time() )
		return true;	//expired
...
function _jr_pack_calc_remain_offers( $offer, $pack, $type = 'user' ) {
	...
	if ( 'user' == $type && ! empty($pack['plan_data'][$offer]) && ! empty($pack['meta'][$offer]) ) {
		$count = $pack['plan_data'][$offer . '_limit'] - $pack['meta'][$offer . '_count'];
	} else {
...
function jr_user_pack_output( $pack ) {
	...
	$remain_jobs = $pack['plan_data'][JR_FIELD_PREFIX.'jobs_limit'] - $pack['meta']['jobs_count'];

	if ( ! $pack['plan_data'][JR_FIELD_PREFIX.'jobs_limit'] ):
		$jobs_count = __('Unlimited', APP_TD);
		$pack_jobs_count = 9999;
	else :
		$jobs_count = ( $remain_jobs >= 0 ? $remain_jobs : 0 ). ( $remain_job_offers > 0 ? ' (+' . $remain_job_offers . __( ' Free', APP_TD ) . ')' : '' );
		$pack_jobs_count = $remain_jobs;
	endif;
...
function jr_new_pack_output( $pack ) {
	...
	if ( $pack['plan_data'][JR_FIELD_PREFIX.'duration'] )
		$echo_pack_duration_expire = sprintf('%s <small>%s</small>',__(' usable within ', APP_TD),  $pack['plan_data'][JR_FIELD_PREFIX.'duration'].__(' days', APP_TD) );
	else 
...
function jr_pack_commons_output( $type, $pack, $display_options = array() ) {
	...
	foreach ( $pack_offers as $key => $p_offer ):
		...
		if ( $limit > 0 ):
			...
			$echo_pack_offers .= sprintf('%s <span class="pack-offer %s"><small>%s</small> %s</span>', ( $echo_pack_offers ? ', ' : '' ), $offer_used_class, $count, $p_offer['name'] );
...
function jr_optional_featured_addons_output( $html, $addons, $args ) {
	...
	if ( empty($pack) || $addons != _jr_featured_addons() )
	 ...
	 foreach ( $addons as $addon ) : 
		...
		//if ( ! empty( $plan_data[$addon][0] ) && $remain_offers ) {
