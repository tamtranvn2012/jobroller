...
// set the resumes per page on the resumes archive listing
function jr_resumes_posts_per_page( $query ) {
	if ( $query->is_main_query() && is_archive(APP_POST_TYPE_RESUME) && ! is_admin() ) {
		set_query_var( 'posts_per_page', jt_get_resumes_per_page() );
	}
}
...
function jr_resume_is_visible( $single = '' ) {
	...
	if ( jr_viewing_resumes_require_subscription() ) {
...
function jr_user_resumes_access( $user_id = 0 ) {
	...
	if ( ! $active_subscription ) {
		...
		// TODO: FAZER O MESMO PARA O VALID_TRIAL
