<?php
	...
	ob_start();
?>
<?php if ( $my_query && $my_query->have_posts() ) : $alt = 1; echo '<div class="section"><h2 class="pagetitle"><span><small class="rss"><a href="'.jr_get_featured_jobs_rss_url().'"><img src="'.get_bloginfo('template_url').'/images/feed.png" title="'.__('Featured Jobs RSS Feed',APP_TD).'" alt="'.__('Featured Jobs RSS Feed',APP_TD).'" /></a></small> '.__('Featured Jobs',APP_TD).' </span></h2><ol class="jobs">'; while ($my_query->have_posts()) : $my_query->the_post(); 
...
?>
		...
		<dd class="title"><strong><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></strong>
			...
			<?php if ( $company ) : ?>
				...
				<?php if ( $compurl = get_post_meta( $my_query->post->ID, '_CompanyURL', true ) ) { ?>
					<a class="hide" href="<?php echo esc_url( $compurl ); ?>" rel="nofollow"><?php echo wptexturize( $company ); ?></a>
				<?php } else { ?>
    ...
    <div class="jobpostdate">Posted    <span><?php echo date_i18n(__('j M',APP_TD), strtotime($my_query->post->post_date)); ?>, <?php echo date_i18n(__('Y',APP_TD), strtotime($my_query->post->post_date)); ?></span></div>
