...
function jr_addons_output( $addons, $args ) {
	...
	foreach ( $addons as $addon ) {
		...
		if ( ! empty( $plan_data[$addon][0] ) && ! _jr_addon_already_active( $addon, $job_id ) ) {
			$option_html = _jr_addon_option( $addon, true,$plan_id, $echo = false, 'free-' );
			if ( $plan_data[$addon.'_duration'][0] != 0 ) {
				$option_html .= sprintf( _n( '%s is included in this plan for %d day.', '%s is included in this plan for %d days.', $plan_data[$addon.'_duration'][0], APP_TD ), APP_Item_Registry::get_title( $addon ), $plan_data[$addon.'_duration'][0] );
			} else {
