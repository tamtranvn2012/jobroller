...
class PluginUpdateChecker {
	...
	public $scheduleName = 'weekly';		// The CRON schedule name
	 ...
	 * @return void
	 */
	function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 168, $optionName = ''){
		$this->metadataUrl = $metadataUrl;
		//$this->pluginFile = plugin_basename($pluginFile);
		$this->pluginFile = plugin_basename($pluginFile) . '/' . $pluginFile .'.php';
	...
	function installHooks(){
		...
		if ( $this->checkPeriod > 0 ){
			...
			if ( !wp_next_scheduled($cronHook) && !defined('WP_INSTALLING') ) {
				...
				wp_schedule_event(time(), $this->scheduleName, $cronHook);
			}
	...
	function _addCustomSchedule($schedules){
		if ( $this->checkPeriod && ($this->checkPeriod > 0) ){
			//$scheduleName = 'every' . $this->checkPeriod . 'hours';
			$schedules[$this->scheduleName] = array(
				'interval' => $this->checkPeriod * 3600, 
				'display' =>  ( 604800 == $this->checkPeriod *3600 ? __( 'Once Weekly', JR_FX_i18N_DOMAIN ) : sprintf( __( 'Every %d hours' , JR_FX_i18N_DOMAIN ), $this->checkPeriod ) ),
			);
		}		
		return $schedules;
	}
	...
	function maybeCheckForUpdates(){
		...
		/* DEBUG *
		//TEMP - uncomment to check for updates instantly
		$shouldCheck = 1;
		/**/
