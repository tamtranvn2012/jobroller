...
class RTMPClient
{
	...
	{
		//echo 'RtmpPacket::TYPE_INVOKE_AMF0='.RtmpPacket::TYPE_INVOKE_AMF0."</br>";
		if($this->listening)
			return;
		if(!$this->socket)
			return;
			...
			if($p = $this->readPacket())
			{
				...
				switch($p->type)
				{
					...
					case RtmpPacket::TYPE_INVOKE_AMF0:
						//echo $p->type."=".RtmpPacket::TYPE_INVOKE_AMF0."which means AMF0(0x14), p=</br>";
						//break;
					case RtmpPacket::TYPE_INVOKE_AMF3: //Invoke
						//echo $p->type."=".RtmpPacket::TYPE_INVOKE_AMF3."which means AMF3(0x11), p=</br>";
						$return = $this->handle_invoke($p);
						if(sizeof($this->operations) == 0)  // ORIGINAL LINE OF CODE
						$stop = true;
						break;
					case 0x16:
						//echo "0x16</br>";
	...
	private function handshake()
	{
		...
		$stream->write("\x80\x00\x03\x02");	//Zero zone? Flex put : 0x80 0x00 0x03 0x02, maybe new handshake style?  - ORIGINAL LINE OF CODE
	...
	private function send_ConnectPacket($connectParams=null)
	{
		$this->sendOperation(
			new RtmpOperation(new RtmpMessage("connect",(object)array(
					...
					"flashVer" => "Android/iOS HTML",
					"swfUrl" => "http://".$_SERVER["HTTP_HOST"]."/",
	...
	private function handle_invoke(RtmpPacket $p)
	{
		...
		if($op->getCall() && $op->getResponse()->isResponseCommand() )
		{
			...
			{
				$data = (object)$data;
				throw new Exception(header('Location: error.php?invalidLicense=true'));
				//$data->description . (isset($data->application)&&!empty($data->application)?" (Application specific message: {$data->application})":'')
				//$data->description . " Application specific message:"
				//header('Location: error.php?invalidLicense=true')
				return;
			}
			return $data;
		}
